...
def main():
    ...
    global rank, world_size, node_num, local_size, local_rank, train_proc_num, node_idx, total_comm_time

    rank = dist.get_rank()
    world_size = dist.get_world_size()
    train_proc_num = world_size
    ...
    if rank==0 : 
        ...
        print('train_loader length : ', math.ceil(1281167/float(args.batch_size * train_proc_num)))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if rank==0: 
            print('Epoch ['+str(epoch+1)+'] time : ' + str(end - start) + ' with loss : ' + str(train_loss))
            print('Total Communication Time : ', str(total_comm_time[0]))
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    train_loader_length = math.ceil(1281167/float(args.batch_size * train_proc_num))
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        total_comm_time[0] = total_comm_time[0] + (end - start)
        #if rank==0: print('Epoch ['+str(epoch+1)+'], iteration['+str(i)+'] total_comm_time : ' + str(total_comm_time[0]))

        optimizer.step()
        n_samples += output.size(0)
        total_loss += (loss.item() * output.size(0))
        #if rank==1: print('n_samples : ', n_samples)
        #if rank==0: print('[i:',i,'] losses.avg : ', losses.avg)
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        if rank==0: print('Loss {loss.val:.4f} ({loss.avg:.4f}) \t Prec@1 {top1.avg:.3f} \t  Prec@5 {top5.avg:.3f}'.format(loss=losses, top1=top1, top5=top5))
